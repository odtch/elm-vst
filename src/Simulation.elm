...
error : String -> Simulation -> Simulation
error text simulation =
    { simulation | errors = simulation.errors ++ [ text ] }
...
addDevice : Id -> DeviceType -> Simulation -> Simulation
addDevice id device_type simulation =
    case Dict.get id simulation.devices of
        Just existing ->
            Debug.crash "already a device with this id"

        Nothing ->
            let
                new_device =
                    { id = id, type_ = device_type }
            in
                { simulation | devices = Dict.insert id new_device simulation.devices }


findDevice : Id -> Simulation -> Maybe Device
findDevice id simulation =
    Dict.get id simulation.devices
...
getDevice id simulation =
    case Dict.get id simulation.devices of
        Nothing ->
            Debug.crash ("device not found " ++ (Basics.toString id))

        Just reference ->
            reference


replaceDevice : Device -> Simulation -> Simulation
replaceDevice device simulation =
    case findDevice device.id simulation of
        Nothing ->
            error "replaceDevice failed because device not found" simulation

        Just existing ->
            --if (existing.type_ == device.type_) then
            { simulation | devices = Dict.insert device.id device simulation.devices }
...
--     case msg of
--         Animate time ->
--             animate time simulation
--
--         MachineMsg id machineMsg ->
--             let
--                 device =
--                     getDevice id simulation
--             in
...
--                     MachineType machine ->
--                         let
--                             ( new_machine, machine_msg ) =
--                                 Machine.update machineMsg machine
--                         in
--                             { simulation | devices = Dict.insert id { device | type_ = (MachineType new_machine) } simulation.devices } ! []
--
--                     _ ->
